...
function it_exchange_manual_purchase_print_add_payment_screen() {
				...
				if ( !empty( $post['userid'] ) || ( !empty( $post['user_login'] ) && !empty( $post['email'] ) ) ) {
					...
					if ( !is_wp_error( $user_id ) ) {
						...
						if ( !empty( $user ) ) {
							...
							foreach ( $post['product_ids'] as $product_id ) {
								if ( !empty( $product_id ) ) {
									if ( ! $product = it_exchange_get_product( $product_id ) ) {
										$error_message = sprintf( __( 'No Product Found - Product ID: %s', 'it-l10n-exchange-addon-manual-purchases' ), $product_id );
										continue;
									}
							...
							if ( empty( $products ) ) {
								$error_message = __( 'No Product Selected', 'it-l10n-exchange-addon-manual-purchases' );
							}
							...
							if ( empty( $error_message ) ) {
								...
								$status_message = sprintf( __( 'Successfully added Manual Purchase. <a href="%s">View Transaction</a> | <a href="%s">View Customer Data</a>', 'it-l10n-exchange-addon-manual-purchases' ), esc_url( $transaction_url ), esc_url( $customer_data_url ) );
							}
						} else {
							$error_message = __( 'No user found.', 'it-l10n-exchange-addon-manual-purchases' );
						}
					} else {
						$error_message = $user_id->get_error_message();
					}
				} else {
					$error_message = __( 'You must select an existing user or create a new one.', 'it-l10n-exchange-addon-manual-purchases' );
				}
			} else {
				$error_message = __( 'You must select products to create a manual purchase.', 'it-l10n-exchange-addon-manual-purchases' );
			}
		} else {
			$error_message = __( 'Error validating security token. Please try again.', 'it-l10n-exchange-addon-manual-purchases' );
		}
	...
	<div class="wrap">
		...
		?>
		<h2><?php _e( 'Add Manual Purchase', 'it-l10n-exchange-addon-manual-purchases' ); ?></h2>
		...
		<div class="it-exchange-add-manual-purchase">
			<div class="it-exchange-add-manual-purchase-user-options">
				<div class="invoice-field-container invoice-field-container-client-type">
					<h3><?php _e( 'New or Existing Customer', 'it-l10n-exchange-addon-manual-purchases' ); ?></h3>
					<label for="it-exchange-user-option-existing"><input type="radio" id="it-exchange-user-option-existing" class="it-exchange-user-option" name="user_option" value="existing" <?php checked( 'existing', $default['user_option'] ); ?> />&nbsp;<?php _e( 'Existing Customer', 'it-l10n-exchange-addon-manual-purchases' ); ?></label>
					<label for="it-exchange-user-option-new"><input type="radio" id="it-exchange-user-option-new" class="it-exchange-user-option" name="user_option" value="new" <?php checked( 'new', $default['user_option'] ); ?> />&nbsp;<?php _e( 'New Customer', 'it-l10n-exchange-addon-manual-purchases' ); ?></label>
				</div>
				<?php
				...
				<div class="it-exchange-add-manual-purchase-user-option-existing <?php echo $existing_hidden; ?>">
					...
					<label for="it-exchange-manual-purchase-existing-username"><?Php _e( 'Username', 'it-l10n-exchange-addon-manual-purchases' ); ?></label>
					<?php 
						$args = array(
				...
				<div class="it-exchange-add-manual-purchase-user-option-add-new <?php echo $new_hidden; ?>">
					<div class="field">
						<label for="it-exchange-manual-purchase-new-username"><?Php _e( 'Username', 'it-l10n-exchange-addon-manual-purchases' ); ?></label><input id="it-exchange-manual-purchase-new-username" type="text" value="<?php echo $default['user_login']; ?>" name="user_login" />
					</div>
					<div class="field">
						<label for="it-exchange-manual-purchase-new-email"><?php _e( 'Email', 'it-l10n-exchange-addon-manual-purchases' ); ?></label><input id="it-exchange-manual-purchase-new-email" type="text" value="<?php echo $default['email']; ?>" name="email" />
					</div>
					<div class="field">
						<label for="it-exchange-manual-purchase-new-first-name"><?php _e( 'First Name', 'it-l10n-exchange-addon-manual-purchases' ); ?></label><input id="it-exchange-manual-purchase-new-first-name" type="text" value="<?php echo $default['first_name']; ?>" name="first_name" />
					</div>
					<div class="field">
						<label for="it-exchange-manual-purchase-new-last-name"><?php _e( 'Last Name', 'it-l10n-exchange-addon-manual-purchases' ); ?></label><input id="it-exchange-manual-purchase-new-last-name" type="text" value="<?php echo $default['last_name']; ?>" name="last_name" />
					</div>
					<div class="field set-password">
						<a href="#"><?php _e( 'Set password manually.', 'it-l10n-exchange-addon-manual-purchases' ); ?></a>
					</div>
					<div class="field hidden random-password">
						<a href="#"><?php _e( 'Let WordPress generate a random password.', 'it-l10n-exchange-addon-manual-purchases' ); ?></a>
					</div>
					<div class="hidden show-password-fields">
					<div class="field">
						<label for="it-exchange-manual-purchase-new-password1"><?php _e( 'Password', 'it-l10n-exchange-addon-manual-purchases' ); ?></label><input id="it-exchange-manual-purchase-new-password1" type="password" value="" name="pass1" />
					<div class="field">
					</div>
						<label for="it-exchange-manual-purchase-new-password2"><?php _e( 'Repeat Password', 'it-l10n-exchange-addon-manual-purchases' ); ?></label><input id="it-exchange-manual-purchase-new-password2" type="password" value="" name="pass2" />
					</div>
					</div>
				</div>
			</div>
			<div class="it-exchange-add-manual-purchase-product-options">
				<h3><?php _e( 'Select Products', 'it-l10n-exchange-addon-manual-purchases' ); ?></h3>
				<div class="it-exchange-add-manual-purchase-product-filter-search clearfix">
					...
					<div id="select-product-type-filter">
						<select id="product-type-filter" name="product_type_filter">
							<option value=""><?php _e( 'View All Product Types', 'it-l10n-exchange-addon-manual-purchases' ); ?></option>
							<?php
						...
						<?php submit_button( __( 'Filter', 'it-l10n-exchange-addon-manual-purchases' ), 'secondary', 'filter_submit' ); ?>
					</div>
					<?php 
					}
					?>
					<div id="select-product-search">
						...
						<?php submit_button( __( 'Search Products', 'it-l10n-exchange-addon-manual-purchases' ), 'secondary', 'search_submit' ); ?>
					</div>
				</div>
				<?php echo it_exchange_manual_purchases_product_listing( $default ); ?>
				<div class="clear"></div>
				<label for="it-exchange-add-manual-purchase-total-paid"><?Php _e( 'Total Paid', 'it-l10n-exchange-addon-manual-purchases' ); ?></label><input id="it-exchange-add-manual-purchase-total-paid" type="text" value="<?php echo $default['total']; ?>" name="total" />
				<div id="it-exchange-add-manual-purchase-description-div" class="field">
					<label for="it-exchange-add-manual-purchase-description"><?php _e( 'Purchase Note', 'it-l10n-exchange-addon-manual-purchases' ); ?></label>
