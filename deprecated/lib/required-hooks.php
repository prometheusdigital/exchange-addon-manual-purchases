...
function it_exchange_manual_purchases_get_default_status_options() {
	$options = array(
		'pending'   => _x( 'Pending', 'Transaction Status', 'it-l10n-exchange-addon-manual-purchases' ),
		'Completed' => _x( 'Paid', 'Transaction Status', 'it-l10n-exchange-addon-manual-purchases' ),
		'refunded'  => _x( 'Refunded', 'Transaction Status', 'it-l10n-exchange-addon-manual-purchases' ),
		'voided'    => _x( 'Voided', 'Transaction Status', 'it-l10n-exchange-addon-manual-purchases' ),
	);
	return $options;
}
...
function it_exchange_manual_purchases_addon_transaction_status_label( $status ) {
	...
	switch ( $status ) {
		...
		case 'succeeded':
		case 'paid':
			return __( 'Paid', 'it-l10n-exchange-addon-manual-purchases' );
			break;
		case 'refunded':
			return __( 'Refunded', 'it-l10n-exchange-addon-manual-purchases' );
			break;
		case 'pending':
			return __( 'Pending', 'it-l10n-exchange-addon-manual-purchases' );
			break;
		case 'voided':
			return __( 'Voided', 'it-l10n-exchange-addon-manual-purchases' );
			break;
		default:
			return __( 'Unknown', 'it-l10n-exchange-addon-manual-purchases' );
	}
...
function it_exchange_manual_purchases_addon_user_row_actions( $actions, $user_object ) {
	...
	$actions['it_exchange_manual_purchase'] = '<a href="' . esc_url( $url ) . '" class="thickbox it-exchange-add-manual-purchase-for-user">' . __( 'Add Product(s)', 'it-l10n-exchange-addon-manual-purchases' ) . '</a>';
...
function it_exchange_manual_purchases_admin_user_products() {
	...
	if ( !empty( $user_id ) ) {
		...
		$url = add_query_arg( $args, get_admin_url() . 'admin-ajax.php' ); 
		$output = '<p><a href="' . esc_url( $url ) . '" class="button button-primary button-large thickbox it-exchange-add-manual-purchase-for-user">' . __( 'Add Product(s)', 'it-l10n-exchange-addon-manual-purchases' ) . '</a></p>';
...
function it_exchange_manual_purchases_add_menu_item() {
	if ( ! empty( $_GET['page'] ) && 'it-exchange-add-manual-purchase' == $_GET['page'] ) {
		...
		add_submenu_page( 'it-exchange', __( 'Add Manual Purchase', 'it-l10n-exchange-addon-manual-purchases' ), __( 'Add Manual Purchase', 'it-l10n-exchange-addon-manual-purchases' ), 'manage_options', $slug, $func );
	}
}
...
function it_exchange_manual_purchases_after_payment_details() {
	...
	<div class="it-exchange-manual-purchase-note clearfix spacing-wrapper">
		<div class="it-exchange-manual-purchase-note-label">
			<?php _e( 'Manual Purchase Note', 'it-l10n-exchange-addon-manual-purchases' ); ?>
		</div>
