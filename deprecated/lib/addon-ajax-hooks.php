...
function it_exchange_manual_purchase_for_user_print_add_products_screen() {	
		...
		if ( check_admin_referer( 'it-exchange-manual-purchase-add-payment', 'it-exchange-manual-purchase-add-payment-nonce' ) ) {
			...
			if ( !empty( $post['product_ids'] ) ) {
				if ( !empty( $post['userid'] ) ) {
					...
					if ( !is_wp_error( $user_id ) ) {
						...
						if ( !empty( $user ) ) {
							...
							if ( empty( $error_message ) ) {
								...
								$status_message = sprintf( __( 'Successfully added Manual Purchase. <a href="%s" target="_top">View Transaction</a> | <a href="%s" target="_top">View Customer Data</a>', 'it-l10n-exchange-addon-manual-purchases' ), esc_url( $transaction_url ), esc_url( $customer_data_url ) );
							}
						} else {
					...
					$error_message = __( 'You must select an existing user to use this page.', 'it-l10n-exchange-addon-manual-purchases' );
				}
	...
	?>
	<title><?php _e( 'Add Manual Purchase', 'it-l10n-exchange-addon-manual-purchases' ); ?></title>
