...
function it_exchange_register_manual_purchases_addon() {
	...
	if ( version_compare( $current_version, '1.7.3', '>=' ) ) {
		$options = array(
			'name'              => __( 'Manual Purchases', 'it-l10n-exchange-addon-manual-purchases' ),
			'description'       => __( 'Add transactions to your customers, manually.', 'it-l10n-exchange-addon-manual-purchases' ),
			'author'            => 'iThemes',
			...
			'labels'      => array(
				'singular_name' => __( 'Manual Purchases', 'it-l10n-exchange-addon-manual-purchases' ),
			),
...
function it_exchange_add_manual_purchases_nag() {
	...
	<div id="it-exchange-manual-purchases-nag" class="it-exchange-nag">
		<?php
		printf( __( 'To use the Manual Purchases add-on for iThemes Exchange, you must be using iThemes Exchange version 1.7.3 or higher. <a href="%s">Please update now</a>.', 'it-l10n-exchange-addon-manual-purchases' ), admin_url( 'update-core.php' ) );
		?>
	</div>
...
function it_exchange_manual_purchases_set_textdomain() {
	load_plugin_textdomain( 'it-l10n-exchange-addon-manual-purchases', false, dirname( plugin_basename( __FILE__  ) ) . '/lang/' );
}
