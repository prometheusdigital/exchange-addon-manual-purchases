 ...
 * @param {Object} config.l10n.modifyTotals
 * @param {Object} config.l10n.purchase
 * @param {Object} config.l10n.summary
 */
(function ( $, Backbone, _, wp, api, common, config ) {
	...
	var Steps = api.View.extend( {
		...
		isCompleted: function ( prototypeOrCallable ) {

			var check;

			if ( prototypeOrCallable.prototype && prototypeOrCallable.prototype instanceof Backbone.View ) {
				check = function ( compare ) {
					if ( compare.prototype ) {
						return compare.prototype instanceof prototypeOrCallable;
					}

					return compare instanceof prototypeOrCallable;
				};
			} else {
				check = prototypeOrCallable;
			}
	...
	var CartView = api.View.extend( {
		...
		initialize: function ( options ) {
			this.views.add( '.notices-container', new CartFeedbackView( { model: this.model } ) );
	...
	var CartFeedbackView = api.View.extend( {

		initialize: function ( options ) {

			itExchange.api.events.on( 'error', this.onAjaxError, this );
			this.model.on( 'change:feedback', this.renderNotices, this );
			this.model.allItems().on( 'change:feedback', this.renderNotices, this );

			api.View.prototype.initialize.call( this, options );
		},

		onAjaxError: function ( xhr ) {
			var error = common.getErrorFromXhr( xhr );

			this.$el.append( this.makeNotice( {
				text: error,
				item: {}
			}, 'error' ) );
		},

		renderNotices: function ( model ) {

			this.$( '.notice-success' ).remove();

			_.forEach( model.get( 'feedback.notices' ), (function ( feedback ) {
				this.$el.append( this.makeNotice( feedback, 'success' ) );
			}).bind( this ) );

			_.forEach( model.get( 'feedback.errors' ), (function ( feedback ) {
				this.$el.append( this.makeNotice( feedback, 'error' ) );
			}).bind( this ) );
		},

		makeNotice: function ( feedback, type ) {
			var $el = $( '<div></div>' );
			$el.addClass( 'notice' ).addClass( 'notice-' + type ).addClass( 'is-dismissible' );
			$el.html( '<p>' + feedback.text + '</p>' );
			this.makeDismissible( $el );

			return $el;
		},

		makeDismissible: function ( $el ) {
			var $button = $( '<button type="button" class="notice-dismiss"><span class="screen-reader-text"></span></button>' ),
				btnText = config.l10n.summary.dismiss || '';
	...
	var AddressSelectView = api.View.extend( {
		...
		onContinue: function ( e ) {
			...
			var current = this.cart.get( this.type + '_address' ), address = this.selected;
		...
		render: function () {
			...
			attr.continue = config.l10n.addressSelect.continue;
	...
	var NewAddressOption = api.View.extend( {
		...
		render: function () {
			this.$el.html( this.template( config.l10n.addressSelect ) );
		}
	...
	var ShippingMethodView = api.View.extend( {
		...
		renderMethods: function () {
			...
			if ( this.shipping.cartWideMethods().length === 1 ) {
	...
	var ModifyTotalsView = api.View.extend( {
		...
		events: {
			...
			'click .save-pre-tax-total'         : 'onSavePreTaxTotal',
			'click .continue'                   : 'onContinue',
			'focusout input[name="preTaxTotal"]': 'formatPreTaxTotalInput'
		},
		...
		formatPreTaxTotalInput: function () {
			var $total = this.$( 'input[name="preTaxTotal"]' );
			$total.val( common.formatPrice( common.getFloatFromString( $total.val() ) ) );
		},

		onSavePreTaxTotal: function ( e ) {
			...
			var totalFloat = common.getFloatFromString( $total.val() );
			$total.prop( 'disabled', true );

			this.cart.save( { 'meta.manual_purchases_pre_tax_total': totalFloat }, {
				success: function () {
					$btn.prop( 'disabled', false );
		...
		render: function () {
			this.$el.html( this.template( config.l10n.modifyTotals ) );
			this.$el.addClass( 'hidden' );
		}
	} );
